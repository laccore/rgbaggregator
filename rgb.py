...
def readSectionRGBFile(filepath, averageRows=None, reporter=None):
...
def aggregateRGBFiles(rgbFileDir, outputPath, averageRows, reporter):
    startTime = time.time()
    rgbRows = []
    totalFiles = 0
    totalRows = 0
    ...
    for rgbFile in rgbFiles:
        ...
        fileRgbRows = readSectionRGBFile(path.join(rgbFileDir, rgbFile), averageRows, reporter)
